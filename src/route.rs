...
pub enum Route {
    ...
    CatchAll { handler: fn(Request) -> BoxFuture<Response, Error>, },
}

impl Route {
    pub fn matches(&self, path: &str) -> bool {
        match self {
            &Route::Literal { path: ref p, .. } => p == path,
            &Route::Regex { ref regex, .. } => regex.is_match(path),
            &Route::CatchAll { .. } => true,
        }
    }
    ...
    fn handle(&self, req: Request) -> BoxFuture<Response, Error> {
        match self {
            &Route::Literal { handler, .. } => handler(req),
            ...
            &Route::CatchAll { handler } => handler(req),
        }
    }
    pub fn render_with(
        &self,
    ...
    ) -> BoxFuture<hyper::server::Response, hyper::Error> {
        let r = Request { request: req };
